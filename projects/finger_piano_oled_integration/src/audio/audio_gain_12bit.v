...
// **理想增益**(文档口径,P8-E 修正):
//
//   ideal(sample_in, level) = 2048 + floor(delta × level / 8),
//   delta = sample_in - 2048
//
// **本模块实现**是上述理想值的 shift/add **量化近似**(P8 §6 逐项移位
// 表:0、δ/8、δ/4、δ/4+δ/8、δ/2、δ/2+δ/8、δ/2+δ/4、δ-δ/8):由于每一项
// 移位各自向 -inf 截断,和不再严格等于先乘后除的 floor,相对理想值的
// 误差 |RTL - ideal| **≤ 1 LSB**,方向与 δ 的符号及 level 组合有关;
// TB 对全部 4096×8 组合用两套独立参考锁定(shift/add 逐位相等 +
// 理想值 ≤1 LSB,见 tb_audio_gain_12bit.v)。
//
// 冻结语义(P8 §3~§8):
...
//     移动);
//   - 无通用乘法器(§6):shift/add 实现,负数右移全部为算术右移
//     ($signed >>>,floor 语义);
//   - 纯组合逻辑,无第二时钟域、无握手(§1/§2);
//   - 不饱和逻辑:12-bit 全范围输入下 |δ| ≤ 2048,任意 level 的
//     |scaled| ≤ |δ|(level 7 = δ-δ/8 同样收缩),且各移位项与理想值
//     偏差 ≤1 LSB,结果 ∈ [0, 4095] 数学上不可越界——TB 对 4096 × 8
//     全部 32768 组合穷举证明无 wrap(§7/§26);刻意不加不可达的饱和
//     分支,避免 XST trim 出新 warning 漂移已冻结的 allowlist(§19)。
//
