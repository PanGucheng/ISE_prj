...
- Spartan-3 系列可选 `project.json.mapPackFactor`（JSON 整数 0～100）控制 MAP `-c` 打包目标；省略时保持默认命令。它不是容量门禁或节省逻辑的证明，改变后须重新实现并阅读时序报告；不得放松约束来满足面积目标。
...
- **冻结命令集之外的辅助脚本（存在，但不是 `ise.ps1` 命令）**：`tools/make-gui-project.ps1` 在 VM 上生成一份**只给人看的** ISE Project Navigator 工程（`...\<工程>\gui-project\<工程>.xise`，含 src/constraints/sim，默认写成 GBK 供 ISE 编辑器正确显示中文）；`tools/convert-encoding.ps1` 做 UTF-8 ↔ GBK 转换。相关 helper 在 `tools/ise-gui-project.ps1`（被 `ise-tools.ps1` dot-source，仅供上述脚本与自测使用）。脚本会把 `includeFiles` 另存一份到 GUI 工程根目录：ISE 14.7 的层次解析器/ISim 在 Simulation 视图下不读 `Verilog Include Directories`，只默认搜索工程目录，否则 `include` 找不到、GUI 无法仿真（实测）。
